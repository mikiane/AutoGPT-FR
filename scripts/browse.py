...
def create_message(chunk, question):
    ...
    return {
        "role": "user",
        "content": f"\"\"\"{chunk}\"\"\" En utilisant le texte ci-dessus, veuillez répondre à la question suivante : \"{question}\" -- si le texte ne permet pas de répondre à la question, veuillez résumer le texte."
    }
...
def summarize_text(text, question):
    ...
    if not text:
        return "Erreur : Pas de texte à résumer"
    ...
    for i, chunk in enumerate(chunks):
        print(f"Résumer un morceau {i + 1} / {len(chunks)}")
